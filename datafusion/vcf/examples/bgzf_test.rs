    ...
    for file_path in remote_files {
        for chunk_size in [16 * 1024 * 1024, 8 * 1024 * 1024, 4 * 1024 * 1024] {
            for concurrent in [8, 4, 2, 1] {
                ...
                println!("Header read successfully. Contains {} contigs, {} filters, {} info fields, and {} format fields",
                         header.contigs().len(),
                         header.filters().len(),
                         header.infos().len(),
                         header.formats().len());
                ...
                if header.contigs().is_empty() {
                    ...
                    println!("File contains information for {} chromosomes/contigs", header.contigs().len());
                }
